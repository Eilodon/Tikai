...
async def get_latest_insight(
    ...
    if not snapshot:
        raise HTTPException(
            status_code=404,
            detail={"error": {"code": "NOT_FOUND", "message": "Chưa có dữ liệu. Vui lòng import file từ TikTok Shop."}}
        )
...
async def recompute_insight(
    ...
    if not locked:
        raise HTTPException(
            status_code=409,
            detail={"error": {"code": "RECOMPUTE_IN_PROGRESS",
    ...
    if not base:
        raise HTTPException(404, detail={"error": {"code": "NOT_FOUND", "message": "Chưa có snapshot để tính lại."}})
    ...
    def _order_to_raw_row(o: Order) -> RawOrderRow:
        return RawOrderRow(
            ...
            sku_id=o.sku_id, sku_name=o.sku_name,
            gmv=o.gmv,
            ...
            creator_id=o.creator_id, creator_name=o.creator_name,
    ...
    if len(rows) > MAX_ORDERS_RECOMPUTE:
        raise HTTPException(
            status_code=413,
            detail={"error": {"code": "TOO_MANY_ORDERS",
    ...
    cogs_map = {k: Decimal(str(v)) for k, v in raw_cogs.items() if v}
